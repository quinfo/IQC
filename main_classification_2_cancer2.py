...
################读取数据##################
# 乳腺癌2（Cryotherapy Dataset）（二分类）
url_cancer2 = "http://archive.ics.uci.edu/ml/machine-learning-databases/breast-cancer-wisconsin/breast-cancer-wisconsin.data"
cancer2 = pd.read_csv(url_cancer2, sep=',', header=None)
cancer2 = pd.DataFrame(cancer2)
cancer2 = cancer2.replace('?', np.nan)
cancer2 = cancer2.dropna(axis=0)
cancer2 = pd.DataFrame(cancer2, dtype=np.int8)
#cancer2 = cancer2.astype(float)
cancer2.data = cancer2.iloc[:, 1:-1].values

cancer2.target = cancer2.iloc[:, 10:].replace(2,0).replace(4, 1).values
print('\n乳腺癌2数据集:>', cancer2.data.shape,
		'\n各类数据量:>',
		list(cancer2.target).count(0),
	  	list(cancer2.target).count(1))
...
for k, (train_index, test_index) in enumerate(skf.split(X, Y)):
	...
	# 准备参数(有调控的作用)
	param = np.ones((1, 9)) * starVal # 会因参数调整
	param_Best = None
	param_Flag = 0
	y = Y[train_index]
	...
	psi_env = (basis(9, 0) + basis(9, 1) + basis(9, 2) + basis(9, 3) + basis(9, 4)
		 + basis(9, 5) + basis(9, 6) + basis(9, 7) + basis(9, 8)).unit()
